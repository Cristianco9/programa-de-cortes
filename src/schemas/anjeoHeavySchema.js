...
export const anjeosHeavySchema = Joi.object({
  ...
  rielInferior: Joi.number().min(10).max(1_200).precision(1).required(),
  rielSuperior: Joi.number().min(10).max(1_200).precision(1).required(),
  anguloTubular: Joi.string().min(9).max(9).required(),
  anguloMedida: Joi.string().min(9).max(11).required(),
